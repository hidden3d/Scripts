...
class CameraEditorDialog(QDialog):
    ...
    def normalize_sensor(self, sensor_str):
        ...
        clean_str = sensor_str.lower().strip().replace(',', '.')
        
        
        clean_str = re.sub(r'[xх*×]', 'x', clean_str)
        
        if self.parent_window and hasattr(self.parent_window, 'debug_logger'):
            self.parent_window.debug_logger.log(f"После замены разделителей: '{clean_str}'")
        ...
        if 'x' in clean_str:
            ...
            if len(parts) == 2:
                try:
                    ...
                    if self.parent_window and hasattr(self.parent_window, 'debug_logger'):
                        self.parent_window.debug_logger.log(f"Нормализованный сенсор (разделитель 'x'): '{result}'")
                    
                    return result
                except ValueError as e:
                    if self.parent_window and hasattr(self.parent_window, 'debug_logger'):
                        self.parent_window.debug_logger.log(f"Ошибка преобразования чисел с разделителем: {e}")
        ...
        if len(numbers) >= 2:
            ...
            try:
                ...
                if self.parent_window and hasattr(self.parent_window, 'debug_logger'):
                    self.parent_window.debug_logger.log(f"Нормализованный сенсор (два числа): '{result}'")

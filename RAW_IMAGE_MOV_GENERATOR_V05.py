...
# Добавляем параметры для Nuke конвертации
NUKE_PATH = "/studio/tools/nuke/nukex"
SELF_DIR = os.path.dirname(os.path.abspath(__file__))

# Добавляем класс для обработки медиа файлов (из quick_time_converter.py)
class MediaFile:
    def __init__(self, file_path):
        self.file_path = file_path

class InputMedia:
    def __init__(self, commandor_format: str):
        self.commandor_format = commandor_format
        parts = commandor_format.split()

        if len(parts) == 1:
            self.nuke_format_input_file_path = commandor_format
            self.frame_range = 'none'
            media_file_path = commandor_format
        elif len(parts) == 3:
            seq_pattern, start_frame, end_frame = parts
            match = re.search(r'%0(\d+)d', seq_pattern)
            if match:
                digit_count = int(match.group(1))
                hash_replacement = '#' * digit_count
                seq_path = seq_pattern.replace(match.group(0), hash_replacement)
                self.nuke_format_input_file_path = seq_path
                self.frame_range = f'{start_frame}-{end_frame}'

                start_frame_int = int(start_frame)
                frame_filename = seq_pattern.replace(match.group(0), f"{start_frame_int:0{digit_count}d}")
                media_file_path = frame_filename
                self.nuke_format_input_file_path = self.nuke_format_input_file_path + " " + self.frame_range 
            else:
                raise ValueError(f"Некорректный формат секвенции: {seq_pattern}")
        else:
            raise ValueError(f"Некорректный формат пути: {commandor_format}")

        self.directory_path = os.path.dirname(self.nuke_format_input_file_path)
        self.media_file = MediaFile(media_file_path)

    def __repr__(self):
        return f"<InputMedia: {self.nuke_format_input_file_path}, Range: {self.frame_range}, Media: {self.media_file}>"

# Добавляем класс для запуска Nuke команд
class NukeCommandRunner(QThread):
    ...
    def run(self):
        try:
            ...
            for line in self.process.stdout:
                ...
                self.log_signal.emit(f"[OUT] {stripped_line}")
                if "+++" in stripped_line:
            ...
            for line in self.process.stderr:
                if self._stop_requested:
                    break
                stripped_line = line.strip()
                self.log_signal.emit(f"[ERR] {stripped_line}")
                if "+++" in stripped_line:
                    self.progress_signal.emit()
            ...
            if self.process and self.process.poll() is None:
                ...
                self.log_signal.emit("[>] Все процессы Nuke были убиты")
...
class ConverterThread(QThread):
    ...
    def __init__(self, folders: List[Tuple[Path, bool, bool, object]], scale_factor: int, video_format: str, 
                 num_threads: int, font_size: float, video_resolution: str, video_codec: str, use_afanasy: bool):
        super().__init__()
    ...
    def process_folder(self, raw_folder: Path, convert_jpg: bool, create_video: bool, tree_item):
        try:
            ...
            if create_video and self._is_running:
                ...
                # Используем Nuke для конвертации в MOV с кодеками mjpeg/prores
                if self.video_format == 'mov' and self.video_codec in ['mjpeg', 'prores']:
                    self.create_video_with_nuke(jpg_folder, raw_folder.parent)
                else:
    ...
    def create_video_with_nuke(self, jpg_folder: Path, output_dir: Path):
        """Создание видео через Nuke для формата MOV с кодеками mjpeg/prores"""
        try:
            ...
            self.log_messages([(f"Начинаем создание MOV через Nuke из {len(images)} кадров", "blue")])
            
            # Создаем временный JSON файл с параметрами конвертации
            video_name = output_dir.name
            output_path = output_dir / f"{video_name}.mov"
            
            # Подготавливаем данные для конвертации
            media_pairs = []
            for img_path in images:
                # Создаем формат, понятный Nuke конвертеру
                nuke_input_path = f"{img_path}"
                media_pairs.append((nuke_input_path, str(output_path)))
            
            # Параметры конвертации по умолчанию (аналогично quick_time_converter.py)
            scale_map = {
                2: "1/2",
                4: "1/4", 
                8: "1/8",
            ...
            scale = scale_map.get(self.scale_factor, "1/2")
            ...
            data = {
                ...
                "input_colorspace": "default",
                "output_colorspace": "sRGB",
                "afanasy": self.use_afanasy
            }
            
            json_path = output_dir / "conversion_data.json"
            with open(json_path, "w", encoding="utf-8") as f:
                json.dump(data, f, indent=4, ensure_ascii=False)
            
            # Запускаем Nuke конвертацию
            nuke_script_path = os.path.join(SELF_DIR, "nuke_quick_time_converter.py")
            if not os.path.exists(nuke_script_path):
                # Создаем упрощенную версию скрипта, если она не существует
                self.create_nuke_converter_script(nuke_script_path)
            ...
            self.log_messages([(f"Запуск Nuke: {' '.join(command)}", "blue")])
            
            # Запускаем процесс
            process = subprocess.run(command, capture_output=True, text=True)
            
            if process.returncode == 0:
                self.log_messages([(f"MOV видео успешно создано: {output_path}", "green")])
            else:
                self.log_messages([(f"Ошибка Nuke конвертации: {process.stderr}", "red")])
            ...
            self.log_messages([(f"Ошибка при создании MOV через Nuke: {e}", "red")])

    def create_nuke_converter_script(self, script_path: str):
        """Создает упрощенный скрипт для Nuke конвертации"""
...
'''
    ...
    def create_video(self, jpg_folder: Path, output_dir: Path):
        """Оригинальный метод создания видео через OpenCV"""
        images = sorted(jpg_folder.glob("*.jpg"))
        if not images:
            self.log_messages([(f"Не найдено JPG файлов в папке {jpg_folder}", "red")])
            return
        ...
        elif self.video_codec == "prores":
            fourcc = cv2.VideoWriter_fourcc(*'apco')
        else:
        ...
        if not out.isOpened():
            ...
            for fallback_codec in fallback_codecs:
                ...
                self.log_messages([(f"Ошибка: не удалось создать видеофайл с любым кодеком", "red")])
                return
...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.scale_factor = 2
        self.video_format = 'mov'  # По умолчанию MOV
        self.num_threads = os.cpu_count()
        self.font_size = 0.8
        self.video_resolution = "Full HD (1920x1080)"
        self.video_codec = "prores"  # По умолчанию prores
        self.use_afanasy = False  # Новая опция
    ...
    def setup_ui(self):
        ...
        self.format_combo.addItems(["mov", "mp4"])  # MOV по умолчанию
        ...
        self.codec_combo.addItems(["prores", "mjpeg", "mp4v", "h264"])  # Добавлены mjpeg и prores
        self.codec_combo.setCurrentText("prores")
        ...
        # Добавляем галочку Afanasy
        self.afanasy_checkbox = QtWidgets.QCheckBox("Использовать Afanasy")
        second_row_layout.addWidget(self.afanasy_checkbox)
    ...
    def start_processing(self):
        ...
        self.use_afanasy = self.afanasy_checkbox.isChecked()  # Получаем состояние галочки
        ...
        self.thread = ConverterThread(selected_folders, self.scale_factor, self.video_format, 
                                     self.num_threads, self.font_size, self.video_resolution, 
                                     self.video_codec, self.use_afanasy)  # Передаем use_afanasy

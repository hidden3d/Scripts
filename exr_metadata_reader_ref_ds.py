...
class CameraEditorDialog(QDialog):
    ...
    def normalize_sensor(self, sensor_str):
        ...
        # Заменяем различные варианты x на стандартный (включая символ умножения ×)
        clean_str = re.sub(r'[xх*×]', 'x', clean_str)
        
        if self.parent_window and hasattr(self.parent_window, 'debug_logger'):
            self.parent_window.debug_logger.log(f"После замены разделителей: '{clean_str}'")
        
        # Пробуем извлечь числа с разделителем x
        if 'x' in clean_str:
        ...
        # Если не удалось разбить по x, ищем числа другими способами
        # Убираем все нечисловые символы, кроме точек и пробелов
    ...
    def open_settings(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            self.metadata_manager.display_metadata(
                ...
                ""  # extension будет определена в display_metadata при необходимости
            )
